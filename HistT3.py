...
arnumdw=ardw.values
headers=list(ardw)
arpp = pd.read_excel('HistRatios2.xlsx','pp',header=0, index_col=None, na_values=['NA'])
arnumpp=arpp.values
arsed = pd.read_excel('HistRatios2.xlsx','sed',header=0, index_col=None, na_values=['NA'])
...
cols = np.arange(16) # ctdad de barras(= esteroles)
ancho = 0.4
...
rect1 = histo.bar(cols,meandevBZ[0,:],ancho, color = 'k', yerr=[np.zeros(16), meandevBZ[1,:]], error_kw=eb, linewidth=1, log=True)
rect2 = histo.bar(cols+ancho,meandevN[0,:], ancho, color = 'silver', yerr = [np.zeros(16),meandevN[1,:]], error_kw = eb2, linewidth=1,edgecolor='grey', log=True)
...
histo.set_ylabel("Trap sterols (%sg/g)"%(u"\u03BC"), size=18,**csfont)
plt.ylim(0,10000)
plt.xlim(0,16)
...
histo.set_xticklabels(headers[1:17], rotation = 90, size=14,**csfont)
...
BZ=[(np.mean(arnumpp[0:sizeBZ,18])),(np.mean(arnumpp[0:sizeBZ,19])),(np.mean(arnumpp[0:sizeBZ,13])),(np.mean(arnumpp[0:sizeBZ,20]))]
print(BZ)
N=[(np.mean(arnumpp[sizeBZ:(sizeBZ+sizeN),19])),(np.mean(arnumpp[sizeBZ:(sizeBZ+sizeN),18])),(np.mean(arnumpp[sizeBZ:(sizeBZ+sizeN),20])),(np.mean(arnumpp[sizeBZ:(sizeBZ+sizeN),13]))]
print(N)
tortaBZ.pie(BZ,startangle=40, colors=colors,wedgeprops={'edgecolor':'k','linewidth':1})
tortaN.pie(N,startangle=245, colors=colorsN,wedgeprops={'edgecolor':'k','linewidth':1})
tortaBZ.set_aspect('equal')
#tortaBZ.set_edgecolor('black')
...
rect4 = histo2.bar(cols,meansedBZ[0,:],ancho, color = 'k', yerr=[np.zeros(16), meansedBZ[1,:]], error_kw=eb, linewidth=1,edgecolor='k', log=True)
rect5 = histo2.bar(cols+ancho+0.1,meansedN[0,:], ancho, color = 'silver',edgecolor='grey', yerr = [np.zeros(16),meansedN[1,:]], error_kw = eb2, linewidth=1, log=True)
# labels y ejes
csfont = {'fontname':'Liberation Sans'}
histo2.set_ylabel('Sediment sterols (ug/g)', size=18,**csfont)
plt.ylim(0,10000)
plt.xlim(0,16)
...
histo2.set_xticklabels(headers[1:17], rotation = 90, position=(0,-0.03), size=14,**csfont)
...
fig1.add_patch(Rectangle((0.02,0.19),0.315,0.78,facecolor = 'none',linewidth=1, edgecolor='k', ls='dotted',joinstyle='round'))
fig1.text(0.05,0.06, r'Fecal sterols', fontsize=18,**csfont)
fig1.add_patch(Rectangle((0.337,0.19),0.362,0.78,facecolor = 'none',linewidth=1, edgecolor='k', ls='dotted',joinstyle='round'))
...
BZ2=[(np.mean(arnumsed[0:sBZ,17])),(np.mean(arnumsed[0:sBZ,18])),(np.mean(arnumsed[0:sBZ,20])),(np.mean(arnumsed[0:sBZ,19]))]
print(BZ2)
N2=[(np.mean(arnumsed[sBZ:(sBZ+sN),18])),(np.mean(arnumsed[sBZ:(sBZ+sN),17])),(np.mean(arnumsed[sBZ:(sBZ+sN),19])),(np.mean(arnumsed[sBZ:(sBZ+sN),20]))]
print(N2)
tortaBZ2.pie(BZ2,startangle=40, colors=colors,wedgeprops={'edgecolor':'k','linewidth':1})
tortaN2.pie(N2,startangle=245, colors=colorsN,wedgeprops={'edgecolor':'k','linewidth':1})
