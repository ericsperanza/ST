...
time1.plot(newx, akima1(newx), 'black', linewidth=5, color = 'blue')
time1.plot(date, flux, 'o', color = 'black', markersize=0, markeredgecolor = 'black')
time1.xaxis.set_major_locator(mdates.MonthLocator(interval = 12))
time1.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
#time1.plot(dd, newy)
plt.ylim(0,110)
plt.yticks(np.arange(0,101,50), size = 22, **csfont)
...
time2.plot(newx, akima2(newx), 'brown',linewidth=5 )
time2.plot(date, copr, 'o', color = 'white', markersize=0,markeredgecolor = 'black')
plt.ylim(0,30)
plt.yticks(np.arange(0,31,15), size = 22, **csfont)
...
time3.plot(newxN, akima3(newxN), 'black', linewidth=5, color = 'blue')
time3.plot(dateN, fluxN, 'o', color = 'black', markersize=0)
time3.xaxis.set_major_locator(mdates.MonthLocator(interval = 12))
time3.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
plt.ylim(0,20)
plt.yticks(np.arange(0,20.1,10), size = 22, **csfont)
#time3.set_ylabel('Vertical flux (g.$\mathregular{cm^{-2}.day^{-1}}$)', size = 22, **csfont)
fig.autofmt_xdate(rotation = 90)
time3.tick_params(axis='x', which='major', labelsize=14)
plt.xticks(size = 28, rotation='horizontal', weight='bold', **csfont)
...
time4.plot(newxN, akima4(newxN), 'brown', linewidth=5)
time4.plot(dateN, coprN, 'o', color = 'white', markersize=0, markeredgecolor = 'black')
plt.ylim(0,250)
plt.yticks(np.arange(0,251,100), size = 22, **csfont)
