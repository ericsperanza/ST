...
for i in range (1,20):
	print("%s: %.2f %s %.2f vs %.2f %s %.2f; t-test: %.4f; r: %.2f, %.4f"%(headers[i+2],(np.mean(WBZ[:,i])),u"\u00b1",(np.std(WBZ[:,1],dtype=float,ddof=1)),(np.mean(CBZ[:,i])),u"\u00b1",(np.std(CBZ[:,i],dtype=float,ddof=1)),(ttest_ind(WBZ[:,i],CBZ[:,i]))[1],(pearsonr(arnum[0:17,2],arnum[0:17,i+2]))[0],(pearsonr(arnum[0:17,2],arnum[0:17,i+2]))[1] ) ).encode('utf-8')
	
print("N:")
print("Warm vs. Cold; p-value; r with flux")
for i in range (1,20):
	print("%s: %.2f %s %.2f vs %.2f %s %.2f; t-test: %.4f; r: %.2f, %.4f"%(headers[i+2],(np.mean(WN[:,i])),u"\u00b1",(np.std(WN[:,1],dtype=float,ddof=1)),(np.mean(CN[:,i])),u"\u00b1",(np.std(CN[:,i],dtype=float,ddof=1)),(ttest_ind(WN[:,i],CN[:,i]))[1],(pearsonr(arnum[18:42,2],arnum[18:42,i+2]))[0],(pearsonr(arnum[18:42,2],arnum[18:42,i+2]))[1] ) ).encode('utf-8')
